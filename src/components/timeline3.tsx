...
const ZoomableTimeline = ({
  ...
  useEffect(() => {
    ...
    function zoomed(event: any) {
      ...
      const domain = x.domain();
      const range = x.range();
      // Generate tick values using interval
      const tickValues = currentInterval.interval.range(domain[0], domain[1]);

      // Map ticks to positions
      const tickPositions = tickValues.map((d) => ({
        date: d,
        pos: x(d),
      }));

      console.log("Current ticks and positions:");
      tickPositions.forEach((t) => {
        console.log(currentInterval.format(t.date), "=>", t.pos);
      });
    ...
    xScaleRef.current = x;
    const initialPivotX = (width - marginLeft - marginRight) / 2 + marginLeft;
  ...
  function updatePivotDateFromScale(position: number) {
    if (xScaleRef.current) {
      const svgX = position + marginLeft; // always offset by marginLeft
      console.log(svgX, "----pivot pos regarding timescacle");
      const date = xScaleRef.current.invert(svgX);
      setPivotDate(date);
    }
  }
  //   function updatePivotDateFromScale() {
  //     if (xScaleRef.current) {
  //       const date = xScaleRef.current.invert(pivotPosition + marginLeft);
  //       console.log(date, "---------from scale");

  //       setPivotDate(date);
  //     }
  //   }
  useEffect(() => {
    console.log("pivot position changed----", pivotPosition);
  }, [pivotPosition]);
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    // Fractional position relative to container
    const relativeX = e.clientX - rect.left; // can be float
    const maxX = width - marginLeft - marginRight;

    // Clamp to bounds
    const newPosition = Math.max(0, Math.min(relativeX - marginLeft, maxX));
  ...
  return (
    <div className="flex flex-col items-center bg-gray-50 overflow-x-hidden min-h-screen">
